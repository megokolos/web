...
public class DBService {
    ...
    private Configuration getMyPostgresConfiguration() {
        ...
        configuration.setProperty("hibernate.hbm2ddl.update", hibernate_hbm2ddl_auto);
    ...
    public long addUser(UsersDataSet usersDataSet) throws DBException {
        try {
            ...
            UsersDAO dao = new UsersDAO(session);
            long id = dao.insertUser(usersDataSet);
            transaction.commit();
            session.close();
            return id;
        } catch (HibernateException e) {
